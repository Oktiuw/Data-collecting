...
for ($i=0;$i<4;$i++) {
    ...
    foreach ($terri as $item) {
        $listeVal=[];
        query:
        $url="https://api.insee.fr/donnees-locales/V0.1/donnees/geo-CHFL@GEO$firstYear"."RP"."$nextYear/{$item->getCodeTypeTerritoire()}-{$item->getCodeTerritoire()}.all";
        ...
        if ($response) {
            ...
            if (array_key_exists('Cellule', $response)) {
                ...
                $stmt = MyPDO::getInstance()->prepare(
                    <<<'SQL'
    UPDATE InfosJob
    SET chauffageCollectif=:nb0, chauffageIndiv=:nb1,chauffageElect=:nb2,chauffageAutre=:nb3
    WHERE codePeriode=:cdP and codeTerritoire=:cdTerri and codeTypeTerritoire=:cdTpTerri
    SQL
                );
                $stmt->execute([":cdP"=>$nextYear,":cdTerri"=>$item->getCodeTerritoire(),':cdTpTerri'=>$item->getCodeTypeTerritoire(),':nb0'=>$listeVal[0],':nb1'=>$listeVal[1],':nb2'=>$listeVal[2],':nb3'=>$listeVal[3]]);
            }
        }
    }
}
